...
typedef struct _wkeHandler
{
	virtual void onBufUpdated (const HDC hdc,int x, int y, int cx, int cy) = 0;
	virtual void onTitleChanged(const wkeString title) = 0;
	virtual void onURLChanged(const wkeString url) = 0;
...
namespace wke
{
    ...
    public:
		...
		virtual void setHandler(wkeHandler *handler) = 0;
		virtual wkeHandler * getHandler() = 0;

		virtual void setUserData(void* userData) = 0;
		virtual void* getUserData() = 0;

		virtual bool gesture(DWORD dwID, DWORD dwFlags, POINT ptsLocation, POINT ptsScreenLocation, unsigned int flags) = 0;
    };
}
