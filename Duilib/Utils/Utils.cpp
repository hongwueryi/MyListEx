...
namespace DuiLib
{
	...
	int CDuiString::Remove(const TCHAR chRemove)
	{
		int nLength = GetLength();

		LPTSTR pszSource = m_pstr;
		LPTSTR pszDest = m_pstr;
		LPTSTR pszEnd = m_pstr + nLength;

		while (pszSource < pszEnd)
		{
			LPTSTR pszNewSource = ::CharNext(pszSource);
			if (*pszSource != chRemove)
			{
				size_t NewSourceGap = (pszNewSource - pszSource);
				LPTSTR pszNewDest = pszDest + NewSourceGap;
				size_t i = 0;
