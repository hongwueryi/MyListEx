	...
	if( pt.x >= rcClient.left + rcCaption.left && pt.x < rcClient.right - rcCaption.right \
		&& pt.y >= rcCaption.top && pt.y < rcCaption.bottom ) {
			...
			if( pControl && _tcsicmp(pControl->GetClass(), _T("ButtonUI")) != 0 && 
				...
				_tcsicmp(pControl->GetClass(), _T("WkeWindowlessUI")) != 0)
				return HTCAPTION;
	}
	...
	case UILIB_ZIPRESOURCE:
		{
			...
			if (!m_lpResourceZIPBuffer) {
				...
				memset(m_lpResourceZIPBuffer, 0, dwSize);
			}
			if (m_lpResourceZIPBuffer != NULL && _tcslen((TCHAR*)m_lpResourceZIPBuffer) <= 0)
			{
...
DuiLib::CPaintManagerUI* WindowImplBase::GetPaintManager()
{
	return &m_PaintManager;
}

void WindowImplBase::AnimateWindow(DWORD times, DWORD flags)
{
	if ( (flags&AW_HIDE) == AW_HIDE)
	{
		m_PaintManager.GetShadow()->ShowShadow(FALSE);
	}
	::AnimateWindow(m_hWnd, times, flags);

	if ((flags&AW_ACTIVATE) == AW_ACTIVATE) {
		RECT rc;
