...
void CListUI::Lineoffset(int offset, bool isScroll)
{
	if (!m_pList)
		return;
	...
	int offsety = 0;
	CScrollBarUI* pVerticalScrollBar = m_pList->GetVerticalScrollBar();
	...
	if (isScroll)
	{
		offsety = offset * pVerticalScrollBar->GetScrollRange() / pVerticalScrollBar->GetRange();
		sz.cy -= offsety;
	}
	else
	{
		offsety = offset * pVerticalScrollBar->GetScrollRange() / m_pList->GetHeight();
		sz.cy += offsety;
	}
	SetScrollPos(sz);
}
...
void CListLabelElementUI::DoEvent(TEventUI& event)
{
    ...
    if( event.Type == UIEVENT_BUTTONDOWN || event.Type == UIEVENT_RBUTTONDOWN )
    {
        if( IsEnabled() ) {
			...
			if (m_pOwner!=NULL)
			{
				CControlUI* control = dynamic_cast<CControlUI*>(m_pOwner);
				if (control)
				{
					control->GetManager()->SendNotify(control, DUI_MSGTYPE_ITEMCLICK);
				}
			}
            Invalidate();
        }
...
bool CListContainerElementUI::IsHot() const
{
...
{
    if( _tcscmp(pstrName, _T("selected")) == 0 ) Select();
	if (_tcscmp(pstrName, _T("needparentupdate")) == 0)
	{
		m_bNeedParentUpdate = (_tcscmp(pstrValue, _T("true")) == 0) ? true:false;
	}
