...
namespace DuiLib {
class CWebBrowserUI;
class CWebBrowserUIExternal : public IDispatch {
	
public:
	CWebBrowserUIExternal(CWebBrowserUI* browser):
		m_browser(browser), m_refNum(1){
	}
	...
	{
		if (lstrcmpW((LPCWSTR)rgszNames[0], _T("CallNativeMethod")) == 0)
		{
			//网页调用window.external.CppCall时，会调用这个方法获取CppCall的ID
			*rgDispId = WM_USER+1001;
		}
		return S_OK;
	}
	...
	{
		if (dispIdMember == WM_USER + 1001)
		{
			//m_browser->GetManager()->SendNotify()
		}
		return S_OK;
	}
	...
	{
		return ::InterlockedIncrement(&m_refNum);
	}

	virtual ULONG STDMETHODCALLTYPE Release(void)
	{
		::InterlockedDecrement(&m_refNum);
		return m_refNum;
	}

	long m_refNum;
	CWebBrowserUI* m_browser;
};
...
CWebBrowserUI::~CWebBrowserUI()
{
	...
	{
		delete m_pWebBrowserEventHandler;
	}
}
...
// 		break;
 	case DISPID_DOCUMENTCOMPLETE:
		DocumentComplete(pDispParams->rgvarg[0].pvarVal);
  		break;
	default:
...
void CWebBrowserUI::DocumentComplete(VARIANT *&url)
{
	if (m_pWebBrowserEventHandler)
	{
		m_pWebBrowserEventHandler->DocumentComplete(url);
	}
}
...
LRESULT CWebBrowserUI::TranslateAccelerator( MSG *pMsg )
{
    if(pMsg->message < WM_KEYFIRST || pMsg->message > WM_KEYLAST)
        return S_FALSE;
	if (pMsg->wParam == 116)//F5刷新键
	{
		//返回s_false让其他消息循环捕获，s_ok则表示已处理不返回给其他消息循环
		return S_FALSE;
	}
...
{
	if (m_browserExternal != NULL)
	{
		*ppDispatch = m_browserExternal;
		return S_FALSE;
	}
	if (m_pWebBrowserEventHandler)
	{
		HRESULT result = m_pWebBrowserEventHandler->GetExternal(ppDispatch);
		if (result != E_NOTIMPL || *ppDispatch != NULL)
		{
			return result;
		}
	}
	m_browserExternal = new CWebBrowserUIExternal(this);
	*ppDispatch = m_browserExternal;
	return S_FALSE;
}
...
CString CWebBrowserUI::ExcuteJS(CString js)
{
	CString result;
	VARIANT ret;
	IDispatch* pFanHtml = this->GetHtmlWindow();

	if (pFanHtml)
	{
		VARIANT jsVar;
		jsVar.vt = VT_BSTR;
		jsVar.bstrVal = js.AllocSysString();

		CWebBrowserUI::InvokeMethod(pFanHtml, T2OLE((LPTSTR)(LPCTSTR)_T("eval")), &ret, &jsVar, 1);
		if (ret.vt == VT_BSTR)
		{
			result = COLE2T(ret.bstrVal);
			//return CString(ret.bstrVal);
		}
		//SysFreeString(jsVar.bstrVal);
	}
	return result;
}

CString CWebBrowserUI::CallJSMethod(CString function, CString parmeter)
{
	CString result;
	VARIANT ret;
	IDispatch* pFanHtml = this->GetHtmlWindow();
	...
	if (pFanHtml)
	{
		...
		avarParams[1].bstrVal = CString(function).AllocSysString();

		avarParams[0].vt = VT_BSTR;
		avarParams[0].bstrVal = CString(parmeter).AllocSysString();

		CWebBrowserUI::InvokeMethod(pFanHtml, T2OLE((LPTSTR)(LPCTSTR)_T("CallJSMethod")), &ret, avarParams, 2);
		if (ret.vt == VT_BSTR)
		{
			result = COLE2T(ret.bstrVal);
		}
		SysFreeString(avarParams[1].bstrVal);
		SysFreeString(avarParams[0].bstrVal);
	}
	return result;
}
