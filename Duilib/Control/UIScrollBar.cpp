...
int CScrollBarUI::GetRange() const
{
	int Range;
	if (!m_bHorizontal) {
		Range = m_rcItem.bottom - m_rcItem.top - m_rcThumb.bottom + m_rcThumb.top - 2 * m_cxyFixed.cx;
	}
	else {
		Range = m_rcItem.right - m_rcItem.left - m_rcThumb.right + m_rcThumb.left - 2 * m_cxyFixed.cy;
	}
	return Range;
}
