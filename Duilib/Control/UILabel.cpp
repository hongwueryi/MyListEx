...
namespace DuiLib
{
	...
	void CLabelUI::SetHoverFont(int index)
	{
		m_iHoverFont = index;
		Invalidate();
	}

	int CLabelUI::GetHoverFont() const
	{
		return m_iHoverFont;
	}
	...
	void CLabelUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
	{
		if( _tcscmp(pstrName, _T("align")) == 0 ) {
			if (_tcsstr(pstrValue, _T("mleft")) != NULL) {
				m_uTextStyle &= ~(DT_CENTER | DT_RIGHT | DT_SINGLELINE);
				m_uTextStyle |= DT_LEFT | DT_EDITCONTROL;
			}
			else if (_tcsstr(pstrValue, _T("left")) != NULL) {
				m_uTextStyle &= ~(DT_CENTER | DT_RIGHT | DT_SINGLELINE);
				m_uTextStyle |= DT_LEFT;
			}
			else if( _tcsstr(pstrValue, _T("csingle")) != NULL ) {
				m_uTextStyle &= ~(DT_LEFT | DT_RIGHT | DT_VCENTER | DT_SINGLELINE );
			...
			else if (_tcsstr(pstrValue, _T("rsingle")) != NULL) {
				m_uTextStyle &= ~(DT_LEFT | DT_CENTER | DT_VCENTER | DT_SINGLELINE);
				m_uTextStyle |= DT_RIGHT;
			}
			else if (_tcsstr(pstrValue, _T("mtop")) != NULL) {
				m_uTextStyle &= ~(DT_BOTTOM | DT_VCENTER | DT_SINGLELINE);
				m_uTextStyle |= DT_TOP | DT_EDITCONTROL;
			}
			...
			else if (_tcsstr(pstrValue, _T("lcenter")) != NULL) {
				m_uTextStyle &= ~(DT_TOP | DT_BOTTOM | DT_CENTER);
				m_uTextStyle |= (DT_LEFT | DT_VCENTER | DT_SINGLELINE);
			}
			else if (_tcsstr(pstrValue, _T("bcenter")) != NULL) {
				m_uTextStyle &= ~(DT_TOP | DT_VCENTER);
				m_uTextStyle |= (DT_BOTTOM | DT_CENTER | DT_SINGLELINE);
			}
			else if (_tcsstr(pstrValue, _T("rcenter")) != NULL) {
				m_uTextStyle &= ~(DT_TOP | DT_BOTTOM | DT_CENTER);
		...
		else if (_tcscmp(pstrName, _T("hoverfont")) == 0) SetHoverFont(_ttoi(pstrValue));
	...
	void CLabelUI::PaintText(HDC hDC)
	{
		...
		DrawText(hDC, rc, clrColor, m_iFont);
	}
	...
	{
		if(bEndelLipSis)	m_uTextStyle |= DT_END_ELLIPSIS;
		else m_uTextStyle &= ~DT_END_ELLIPSIS;
	}

	void CLabelUI::DrawText(HDC hDC, RECT rc, DWORD color, int uFont)
	{
		if (!GetEnabledEffect())
		{
			...
			if (IsEnabled()) {
				if (m_bShowHtml)
					CRenderEngine::DrawHtmlText(hDC, m_pManager, rc, m_sText, color, \
						NULL, NULL, nLinks, /*DT_SINGLELINE | */m_uTextStyle);
				else
					CRenderEngine::DrawText(hDC, m_pManager, rc, m_sText, color, \
						uFont, /*DT_SINGLELINE |*/ m_uTextStyle);
			}
			else {
				if (m_bShowHtml)
					CRenderEngine::DrawHtmlText(hDC, m_pManager, rc, m_sText, color, \
		...
		else if (GetTextColor1() == -1) {
			//duilib 之前支持GDI渐变的文字处理，这里简化不增加新的属性支持
			//GDI+暂时不支持htm
			CRenderEngine::DrawText(hDC, m_pManager, rc, m_TextValue, color, \
				uFont, /*DT_SINGLELINE |*/ m_uTextStyle, true, GetTransText());
		}
		else {
			Font	nFont(hDC, m_pManager->GetFont(uFont));
			...
			LinearGradientBrush nLineGrBrushB(Point(GetGradientAngle(), 0), Point(0, nGradientLength), _MakeRGB(GetTransText(), color), _MakeRGB(GetTransText1(), GetTextColor1() == -1 ? color : GetTextColor1()));
