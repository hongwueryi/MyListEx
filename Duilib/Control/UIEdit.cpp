...
namespace DuiLib
{
	...
	LRESULT CEditWnd::HandleMessage(UINT uMsg, WPARAM wParam, LPARAM lParam)
	{
		...
		if( uMsg == WM_KILLFOCUS ) lRes = OnKillFocus(uMsg, wParam, lParam, bHandled);
			...
			if( m_hBkBrush == NULL ) {
				if (!m_pOwner->m_dwEditbkImage.IsEmpty())
				{
					const TImageInfo* data = m_pOwner->GetManager()->GetImageEx(
						m_pOwner->m_dwEditbkImage, NULL, 0);
					m_hBkBrush = ::CreatePatternBrush(data->hBitmap);
				}else{
	...
	void CEditUI::SetNativeEditBkImage(LPCTSTR dwBkImage)
	{
		m_dwEditbkImage = dwBkImage;
	}
	LPCTSTR CEditUI::GetNativeEditBkImage()
	{
		return m_dwEditbkImage;
	}
	...
	void CEditUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
	{
		...
		else if (_tcscmp(pstrName, _T("nativebkimage")) == 0) {
			SetNativeEditBkImage(pstrValue);
		}
	...
	void CEditUI::PaintText(HDC hDC)
	{
		...
		if (m_sText.IsEmpty() && m_tipText.IsEmpty()) return;

		DWORD mCurTextColor = m_dwTextColor;
		CDuiString sText;
		if (m_sText.IsEmpty()) {
			sText = m_tipText;
			mCurTextColor = m_tipColor;
		}
		else
		{
			sText = m_sText;
	...
	DuiLib::CDuiString CEditUI::GetTipText()
	{
		return m_tipText;
	}

	void CEditUI::SetTipText(CDuiString tipText)
	{
		m_tipText = tipText;
	}
	...
	void CEditUI::SetTipColor(DWORD tipColor)
	{
		m_tipColor = tipColor;
	}
