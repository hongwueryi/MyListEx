        ...
        if( _tcsicmp(pstrClass, _T("Window")) == 0 ) {
            if( pManager->GetPaintWindow() ) {
                ...
                for( int i = 0; i < nAttributes; i++ ) {
					...
					else if (_tcscmp(pstrName, _T("animateimage")) == 0) {
						pManager->SetAnimateImage(pstrValue);
					}
					else if (_tcscmp(pstrName, _T("moveparentwnd")) == 0) {
						pManager->SetMoveParentWnd(_tcscmp(pstrValue, _T("true")) == 0);
					}
					else if (_tcscmp(pstrName, _T("alphabktrans")) == 0) {
						pManager->SetAlphaBktrans(_tcscmp(pstrValue, _T("true")) == 0);
					}
                }
            }
        }
    ...
    for( CMarkupNode node = pRoot->GetChild() ; node.IsValid(); node = node.GetSibling() ) {
		...
		//树控件XML解析
            ...
            case 7:
				...
				else if (_tcscmp(pstrClass, DUI_CTR_LABELEX) == 0)			  pControl = new CLabelUIEx;
				break;
            case 8:
            ...
            case 10:
				...
				else if (_tcsicmp(pstrClass, DUI_CTR_PROCRESSEX) == 0)		   pControl = new CProgressExUI;
                break;
			case 11:
				if (_tcsicmp(pstrClass, DUI_CTR_CHILDLAYOUT) == 0)			  pControl = new CChildLayoutUI;
				if (_tcsicmp(pstrClass, DUI_CTR_SQQUENCEPICTURE) == 0)		  pControl = new CSequencePictureUI;
				break;
			case 12:
				if (_tcsicmp(pstrClass, DUI_CTR_CUSTOMBUTTON) == 0)			  pControl = new CCustomButtonUI;
				break;
            case 14:
				...
				else if (_tcsicmp(pstrClass, DUI_CTR_BUTTONSWITCHEX) == 0)		   pControl = new CButtonSwitchExUI;
                break;
            case 15:
                if( _tcsicmp(pstrClass, DUI_CTR_LISTTEXTELEMENT) == 0 )        pControl = new CListTextElementUI;
				else if (_tcsicmp(pstrClass, DUI_CTR_ROTATEANIMATE) == 0)        pControl = new CRotateAnimateUI;
				else if (_tcsicmp(pstrClass, DUI_CTR_ANIMATIONOPTION) == 0)       pControl = new CAnimationOptionUI;
                break;
            case 16:
