...
HBITMAP CRenderEngine::m_hBitmap;
...
bool CRenderEngine::DrawImage(HDC hDC, CPaintManagerUI* pManager, const RECT& rcItem, const RECT& rcPaint, 
					  TDrawInfo& drawInfo, DWORD dwAngle)
{
	// 1、aaa.jpg
	...
	if (0 != dwAngle)
	{
		hBitmap = CreateBitmapFromHBITMAP(drawInfo.pImageInfo->hBitmap, dwAngle);
		DeleteObject(m_hBitmap);
		m_hBitmap = hBitmap;
	}
	else
	{
		hBitmap = drawInfo.pImageInfo->hBitmap;
	}

	DrawImage(hDC, hBitmap, rcDest, rcPaint, drawInfo.rcBmpPart, drawInfo.rcCorner,
...
void CRenderEngine::DrawText(HDC hDC, CPaintManagerUI* pManager, RECT& rc, LPCTSTR pstrText, DWORD dwTextColor, int iFont, UINT uStyle, bool gdiplug, int alpha)
{
    ASSERT(::GetObjectType(hDC)==OBJ_DC || ::GetObjectType(hDC)==OBJ_MEMDC);
	...
	else
	{
		...
		graphics.SetSmoothingMode(SmoothingMode::SmoothingModeAntiAlias);//降噪
		...
		Gdiplus::SolidBrush brush(Gdiplus::Color(alpha, GetBValue(dwTextColor), GetGValue(dwTextColor), GetRValue(dwTextColor)));
...
HBITMAP CRenderEngine::CreateBitmapFromHBITMAP(HBITMAP hBitmap, DWORD dwAngle)
{
	...
	Gdiplus::Bitmap*  pImg = new Gdiplus::Bitmap(bmp.bmWidth, bmp.bmHeight, PixelFormat32bppPARGB);
	if (!pImg)
	return hRetBitmap;
	int xCenter = pBitmap->GetWidth() / 2;
	int yCenter = pBitmap->GetHeight() / 2;
	Gdiplus::Graphics graph(pImg);
	PointF pt(xCenter, yCenter);
	Matrix matrix;
	matrix.RotateAt(dwAngle, pt);
	graph.SetTransform(&matrix);
	graph.DrawImage(pBitmap, 0, 0, pBitmap->GetWidth(), pBitmap->GetWidth());
	Color cr;
	pImg->GetHBITMAP(cr, &hRetBitmap);
	if (pBitmap)
	{
		delete[](pBitmap);
		pBitmap = NULL;
