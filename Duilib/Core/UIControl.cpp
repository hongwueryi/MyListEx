...
void CControlUI::SetForeImage(LPCTSTR pStrImage)
{
	...
	DrawImage(NULL, m_diFore);
	if (m_bFloat && m_cxyFixed.cx == 0 && m_cxyFixed.cy == 0 && m_diFore.pImageInfo) {
		m_cxyFixed.cx = m_diFore.pImageInfo->nX;
		m_cxyFixed.cy = m_diFore.pImageInfo->nY;
	}
	Invalidate();
}

LPCTSTR CControlUI::GetBkImage2()
{
	return m_diFore.sDrawString;
}

void CControlUI::SetBkImage2(LPCTSTR pStrImage)
{
	if (m_diFore.sDrawString == pStrImage && m_diFore.pImageInfo != NULL) return;
	m_diFore.Clear();
	m_diFore.sDrawString = pStrImage;
...
bool CControlUI::DrawImage(HDC hDC, TDrawInfo& drawInfo, DWORD dwAngle)
{
	return CRenderEngine::DrawImage(hDC, m_pManager, m_rcItem, m_rcPaint, drawInfo, dwAngle);
}
...
CDuiString CControlUI::GetAttribute(LPCTSTR pstrName)
{
	...
	{
		RECT trp = GetPos();
		sValue.Format(_T("%d,%d,%d,%d"), trp.left, trp.top, trp.right, trp.bottom);
		//sValue = CUIUtil::Vertex42String(GetPos() );
	}
	else if (_tcscmp(pstrName, _T("relativepos")) == 0)
	{
		RECT trp = GetRelativePos();
		sValue.Format(_T("%d,%d,%d,%d"), trp.left , trp.top, trp.right, trp.bottom);
	}
	else if (_tcscmp(pstrName, _T("padding")) == 0)
	{
		RECT rcPadding = GetPadding();
		sValue = CUIUtil::Rect2String(rcPadding);
	}
	else if (_tcscmp(pstrName, _T("bkcolor")) == 0 || _tcscmp(pstrName, _T("bkcolor1")) == 0)
	{
		sValue = CUIUtil::Color2String(GetBkColor());
	}
	else if (_tcscmp(pstrName, _T("bkcolor2")) == 0)
	{
		sValue = CUIUtil::Color2String(GetBkColor2());
	}
	else if (_tcscmp(pstrName, _T("bkcolor3")) == 0)
	{
		sValue = CUIUtil::Color2String(GetBkColor3());
	}
	else if (_tcscmp(pstrName, _T("bordercolor")) == 0)
	{
		sValue = CUIUtil::Color2String(GetBorderColor());
	}
	else if (_tcscmp(pstrName, _T("focusbordercolor")) == 0)
	{
		sValue = CUIUtil::Color2String(GetFocusBorderColor());
	}
	else if (_tcscmp(pstrName, _T("colorhsl")) == 0)
	{
		sValue = CUIUtil::Bool2String(IsColorHSL());
	}
	else if (_tcscmp(pstrName, _T("bordersize")) == 0)
	{
		sValue = CUIUtil::Int2String(GetBorderSize());
	}
	else if (_tcscmp(pstrName, _T("borderround")) == 0)
	{
		sValue = CUIUtil::Size2String(GetBorderRound());
	}
	...
	{
		sValue = CUIUtil::Int2String(GetFixedWidth());
	}
	else if (_tcscmp(pstrName, _T("height")) == 0)
	{
		sValue = CUIUtil::Int2String(GetFixedHeight());
	}
	else if (_tcscmp(pstrName, _T("minwidth")) == 0)
	{
		sValue = CUIUtil::Int2String(GetMinWidth());
	}
	else if (_tcscmp(pstrName, _T("minheight")) == 0)
	{
		sValue = CUIUtil::Int2String(GetMinHeight());
	}
	else if (_tcscmp(pstrName, _T("maxwidth")) == 0)
	{
		sValue = CUIUtil::Int2String(GetMaxWidth());
	}
	else if (_tcscmp(pstrName, _T("maxheight")) == 0)
	{
		sValue = CUIUtil::Int2String(GetMaxHeight());
	}
	...
	{
		sValue = CUIUtil::Bool2String(IsEnabled());
	}
	else if (_tcscmp(pstrName, _T("mouse")) == 0)
	{
		sValue = CUIUtil::Bool2String(IsMouseEnabled());
	}
	else if (_tcscmp(pstrName, _T("visible")) == 0)
	{
		sValue = CUIUtil::Bool2String(IsVisible());
	}
	else if (_tcscmp(pstrName, _T("float")) == 0)
	{
		sValue = CUIUtil::Bool2String(IsFloat());
	}
	...
	{
		sValue = CUIUtil::Bool2String(IsContextMenuUsed());
	}
	else if (_tcscmp(pstrName, _T("paintfocus")) == 0)
	{
		//sValue = CUIUtil::Bool2String(GetPaintFocus());
	}
	else if (_tcscmp(pstrName, _T("scale")) == 0)
	{
		//sValue = CUIUtil::Vertex42String(GetScale());
	}
	else if (_tcscmp(pstrName, _T("offset")) == 0)
	{
	//	sValue = CUIUtil::Vertex42String(GetOffset());
	}
	else if (_tcscmp(pstrName, _T("shear")) == 0)
	{
	//	sValue = CUIUtil::Vertex42String(GetShear());
	}
	else if (_tcscmp(pstrName, _T("alphavalue")) == 0)
	{
	//	sValue = CUIUtil::Int2String(GetAlphaValue());
	}
	return sValue;
}
...
void CControlUI::PaintBkImage2(HDC hDC)
{
	DrawImage(hDC, m_diFore);
}
