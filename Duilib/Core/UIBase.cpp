...
BOOL CRect::PtInRect(POINT Pos)
{
	return Pos.x <= this->right && Pos.x >= this->left && Pos.y <= this->bottom && Pos.y >= this->top;
}
...
void CWindowWnd::CenterWindow(BOOL isScreenCenter)
{
    ASSERT(::IsWindow(m_hWnd));
	...
	if( hWndCenter == NULL || IsIconic(hWndCenter) || isScreenCenter)
		rcCenter = rcArea;
	else
...
LRESULT CALLBACK CWindowWnd::__WndProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    if( pThis != NULL && IsWindow(pThis->GetHWND())) {
