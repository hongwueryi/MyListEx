...
namespace DuiLib {
	...
	void tagTDrawInfo::Clear()
	{
		...
		sImageName.Empty();
		pImageInfo = NULL;
		rcDestOffset = rcBmpPart = rcCorner = CRect(0,0,0,0);
		bHole = false;
		bTiledX = false;
		bTiledY = false;
	...
	CPaintManagerUI::CPaintManagerUI() :
		...
		m_alphaBktrans(FALSE),
		m_moveParentWnd(FALSE)
	{
		...
		if (m_hUpdateRectPen == NULL) {
			...
			::InitCommonControls();
			HMODULE module = LoadLibraryEx(_T("msimg32.dll"), NULL, LOAD_LIBRARY_SEARCH_SYSTEM32);
			if (module == NULL)
			{
		...
		m_ptLastPointPos.x = m_ptLastPointPos.y = -1;
	}
	...
	bool CPaintManagerUI::PreMessageHandler(UINT uMsg, WPARAM wParam, LPARAM lParam, LRESULT& /*lRes*/)
	{
		...
		case WM_KEYDOWN:
		{
			...
			if (wParam == VK_TAB) {
				...
				if (m_pFocus && m_pFocus->IsVisible() && m_pFocus->IsEnabled() && _tcsstr(m_pFocus->GetClass(), _T("WkeWindowlessUI")) != NULL) {
					return false;
				}
		...
		case WM_SYSKEYDOWN:
		{
			...
			//ALt+空格会导致管家卡死（同时两个线程触发SC_KEYMENU）
			//子窗口是否需要WM_SYSKEYDOWN这个消息？
			if (wParam == VK_F4)
			{
				return FALSE;
			}
			if (::GetParent(this->m_hWndPaint) != NULL && IsWindow(::GetParent(this->m_hWndPaint)))
			{
				//::PostMessage(::GetParent(this->m_hWndPaint), WM_SYSKEYDOWN, wParam, lParam);
			}
			return TRUE;
		}
		break;
	...
	bool CPaintManagerUI::MessageHandler(UINT uMsg, WPARAM wParam, LPARAM lParam, LRESULT& lRes)
	{
		...
		case WM_PAINT:
		{
			...
			if (m_bFocusNeeded && !::IsIconic(GetPaintWindow())) {
				SetNextTabControl();
			}
			//
			// Render screen
				...
				if (m_bOffscreenPaint)
				{
					...
					if (m_bAlphaBackground) {
						if (m_hbmpBackground == NULL) {
							...
							//解决在高DPI下透明显示的问题，临时方案
							HWND ownerWnd = GetWindowOwner(m_hWndPaint);
							if (!ownerWnd)
							{
							...
							{
								POINT ownerClient = { rcClient.left,rcClient.top };
								ScreenToClient(ownerWnd, &ownerClient);
								HDC ownerDC = GetDC(ownerWnd);
								::BitBlt(m_hDcBackground, ps.rcPaint.left, ps.rcPaint.top, ps.rcPaint.right - ps.rcPaint.left,
									ps.rcPaint.bottom - ps.rcPaint.top, ownerDC, ownerClient.x, ownerClient.y, SRCCOPY);
							}
						}
						else
							::SelectObject(m_hDcBackground, m_hbmpBackground);
		...
		case WM_PRINTCLIENT:
		{
			if ((!m_pRoot || m_pRoot->GetPos().right == 0) && m_animateImage.GetLength() > 0)
			{
				//此时窗口还没有初始化，没有执行WM_PAINT
				const TImageInfo* data = this->GetImageEx((LPCTSTR)m_animateImage, NULL, 0);
				if (!data)
					return true;
				RECT rcClient;
				::GetClientRect(m_hWndPaint, &rcClient);
				...
				CRenderEngine::DrawImage((HDC)wParam, data->hBitmap, rcClient, rcClient, rcBmpPart, { 0,0,0,0 }, data->alphaChannel);
				return true;
			}
			RECT rcClient;
			::GetClientRect(m_hWndPaint, &rcClient);
		...
		case WM_MOUSEMOVE:
		{
			if (m_moveParentWnd && ptmove.x > 0)
			{
				HWND parentWnd = ::GetParent(m_hWndPaint);
				if (parentWnd != NULL && IsWindow(parentWnd))
				{
					GetCursorPos(&pemove);                              
					if (wParam == MK_LBUTTON)             
					{
						remove.left = rtmove.left + (pemove.x - ptmove.x);    
						remove.top = rtmove.top + (pemove.y - ptmove.y); 
						DUI__Trace(_T("POINT:%d-- %d"), remove.left, remove.top);
						MoveWindow(parentWnd, remove.left, remove.top, remove.right, remove.bottom, true);
						return 0;
					}
		...
		case WM_LBUTTONDOWN:
		{
			...
			if (m_moveParentWnd)
			{
				...
				if (parentWnd != NULL && IsWindow(parentWnd))
				{
					if (_tcsicmp(pControl->GetClass(), _T("ButtonUI")) != 0
						&& _tcsicmp(pControl->GetClass(), _T("LabelUI")) != 0
						&& _tcsicmp(pControl->GetClass(), _T("CheckBoxUI")) != 0
						&& _tcsicmp(pControl->GetClass(), _T("ComboUI")) != 0
						&& _tcsicmp(pControl->GetClass(), _T("ListLabelElementUI")) != 0
						&& _tcsicmp(pControl->GetClass(), _T("ScrollBarUI")) != 0)
					{
						CControlUI* parentControl = pControl;
						//如果有父控件是可以滚动的，则不再支持拖动
						BOOL enableDrag = TRUE;
						while (parentControl != NULL)
						{
							if (parentControl->GetInterface(_T("IContainer")) != NULL)
							{
								if (((CContainerUI*)parentControl)->GetVerticalScrollBar() != NULL || ((CContainerUI*)parentControl)->GetHorizontalScrollBar() != NULL)
								{
									enableDrag = FALSE;
									break;
								}
						...
						{
							GetCursorPos(&ptmove);
							GetWindowRect(parentWnd, &rtmove);
							remove.right = rtmove.right - rtmove.left;
							remove.bottom = rtmove.bottom - rtmove.top;
						}
					}
				}
			}
		...
		case WM_MOUSEWHEEL:
		{
			...
			event.wKeyState = MapKeyState();
			event.dwTimestamp = ::GetTickCount();
			if (_tcsicmp(pControl->GetClass(), _T("WkeWindowlessUI")) == 0){ event.wParam = wParam; }
			pControl->Event(event);
		...
		case WM_GESTURENOTIFY:
		{
			...
			GESTURECONFIG gc = {
				GID_PAN,              // gesture ID
				GC_PAN_WITH_SINGLE_FINGER_VERTICALLY | GC_PAN_WITH_SINGLE_FINGER_HORIZONTALLY, // settings related to gesture ID that are to be 
								// turned on
								GC_PAN_WITH_GUTTER | GC_PAN_WITH_INERTIA              // settings related to gesture ID that are to be 
												// turned off
			};
			BOOL bResult = SetGestureConfig(
				m_hWndPaint,                 // window for which configuration is specified  
		...
		case WM_GESTURE:
		{
			...
			ScreenToClient(m_hWndPaint, &gesturePoint);
			

			if (GID_BEGIN == gi.dwID)
			{
				ptmove = { 0,0 };
				...
				CControlUI* pControl = FindControl(gesturePoint);
				if (pControl == NULL)
				{
					break;
				}
				BOOL gestureScroll = FALSE;
				CControlUI* parentControl = pControl;
				//如果有父控件是可以滚动的，则不再支持拖动
				while (parentControl!=NULL)
				{
			...
			else if (GID_END == gi.dwID) {
				ptmove = { 0,0 };
				DUI__Trace(_T("end"));
			}
			else if (GID_PAN == gi.dwID)
			{
				if (GF_BEGIN == gi.dwFlags)
				{
					m_ptLastPointPos.x = gi.ptsLocation.x;
					m_ptLastPointPos.y = gi.ptsLocation.y;
					ScreenToClient(m_hWndPaint, &m_ptLastPointPos);
				}
				...
				{
					if (m_moveParentWnd && ptmove.x > 0 && ptmove.y > 0)
					{
						HWND parentWnd = ::GetParent(m_hWndPaint);
						if (parentWnd != NULL && IsWindow(parentWnd))
						{
							//ptmove 手势初始的位置 rtmove窗口的初始位置 remove:right窗口宽度bottom窗口高度
							remove.left = rtmove.left + (gestureScreenPoint.x - ptmove.x);
							remove.top = rtmove.top + (gestureScreenPoint.y - ptmove.y);
							MoveWindow(parentWnd, remove.left, remove.top, remove.right, remove.bottom, false);
							DUI__Trace(_T("moveing:%d %d %d %d"), remove.left, remove.top, remove.right, remove.bottom);
						}
					}
					else
					{
						...
						if (pControl != NULL)
						{
							...
							{
								int zDelta = m_ptLastPointPos.y - gesturePoint.y;
								TEventUI event = { 0 };
								if (_tcsicmp(pControl->GetClass(), _T("ScrollBarUI")) == 0)
								{
									event.wParam = 1;
								}
								else
								...
								event.dwTimestamp = ::GetTickCount();
								pControl->Event(event);
								::SendMessage(m_hWndPaint, WM_MOUSEMOVE, 0, (LPARAM)MAKELPARAM(gesturePoint.x, gesturePoint.y));
							}
						}
					}
					m_ptLastPointPos = gesturePoint;
				}
			}
...
//				//RaiseException(STATUS_INVALID_CRUNTIME_PARAMETER, 0, 0, NULL);
//			}
//#endif
		}
	}
	...
	bool CPaintManagerUI::SetTimer(CControlUI* pControl, UINT nTimerID, UINT uElapse)
	{
		...
		m_uTimerID = (++m_uTimerID);
		if ( m_uTimerID >= (0XFFFFFFFF-1) )
		{
			m_uTimerID = 0x1000;
		}
	...
	bool CPaintManagerUI::SetNextTabControl(bool bForward)
	{
		...
		if (pControl != NULL) this->SetFocus(pControl);
		m_bFocusNeeded = false;
		return true;
	}
	...
	void CPaintManagerUI::SetAnimateImage(LPCTSTR pStrImage)
	{
		m_animateImage = pStrImage;
	}

	void CPaintManagerUI::SetMoveParentWnd(BOOL move)
	{
		m_moveParentWnd = move;
	}

	void CPaintManagerUI::SetAlphaBktrans(bool alphaBktrans)
	{
		m_alphaBktrans = alphaBktrans;
	}
