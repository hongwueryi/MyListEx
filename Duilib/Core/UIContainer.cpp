...
namespace DuiLib
{
		...
		m_bScrollProcess(false)
	{
		...
		m_hIco = NULL;//add by zhouxing 2016/3/31
		m_bIcon = false;//add by zhouxing 2016/3/31
	}
	...
	void CContainerUI::DoEvent(TEventUI& event)
	{
		...
		if( m_pVerticalScrollBar != NULL && m_pVerticalScrollBar->IsVisible() && m_pVerticalScrollBar->IsEnabled() )
		{
			...
			else if (event.Type == UIEVENT_SCROLL)
			{
				Lineoffset(event.lParam ,event.wParam);
				return;
			}
		}
	...
	void CContainerUI::Lineoffset(int offset, bool isScroll)
	{
		SIZE sz = GetScrollPos();
		if (isScroll)
		{
			int offsety = offset * m_pVerticalScrollBar->GetScrollRange() / m_pVerticalScrollBar->GetRange();
			sz.cy -= offsety;
		}
		else
		{
			int offsety = offset * m_pVerticalScrollBar->GetScrollRange() / GetHeight();
			sz.cy += offsety;
		}
		SetScrollPos(sz);
	}
	...
	void CContainerUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
	{
		...
		else if (_tcscmp(pstrName, _T("icon")) == 0)//add by zhouxing 2016/3/31
		{
			m_bIcon = (_tcscmp(pstrValue, _T("true")) == 0) ? true : false;
		}
	...
	void CContainerUI::DoPaint(HDC hDC, const RECT& rcPaint)
	{
		...
		if (m_bIcon)//add by zhouxing 2016/3/31
		{
			// 			if( !m_hIco) return ;
			PaintIco(hDC);
		}
	...
	void CContainerUI::PaintIco(HDC hDC)//add by zhouxing 2016/3/31
	{
		int nW = m_rcItem.right - m_rcInset.right - m_rcItem.left + m_rcInset.left;
		::DrawIconEx(hDC, m_rcItem.left, m_rcItem.top, m_hIco, nW, nW, 0, 0, DI_NORMAL);

	}

	void CContainerUI::SetLocalIcon(HICON hIco)//add by zhouxing 2016/3/31
	{
		m_hIco = hIco;
	}
